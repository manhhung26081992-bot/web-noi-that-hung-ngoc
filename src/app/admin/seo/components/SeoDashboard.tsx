...
export default function SeoDashboard() {
  ...
  const score = buildSeoScore({ overview, health, searchConsoleConnected, googleAdsConnected });

  return <main className={`${styles.dashboard} ${darkMode ? styles.dark : ''}`}>
    <header className={styles.hero}><div><p className={styles.eyebrow}>Nội Thất Hùng Ngọc</p><h1>SEO Dashboard v3.1</h1><p>AI SEO Manager: phân tích dữ liệu, gợi ý việc hằng ngày và theo dõi tiến độ SEO thực dụng.</p></div><div className={styles.heroActions}><button className={styles.secondaryButton} onClick={() => setDarkMode((value) => !value)}>{darkMode ? 'Light Mode' : 'Dark Mode'}</button><button className={styles.primaryButton} onClick={actions.reload}>Làm mới</button></div></header>
