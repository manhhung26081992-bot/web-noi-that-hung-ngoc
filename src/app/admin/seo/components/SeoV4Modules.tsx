...
export function buildAiDailyBrief({ overview, health, productSeoItems, logs, searchConsoleConnected }: { overview: SeoOverview | null; health: SeoHealthSnapshot | null; productSeoItems: ProductSeoItem[]; logs: SeoLog[]; searchConsoleConnected: boolean }): AiDailyBrief[] {
  ...
  const brief: AiDailyBrief[] = [
    ...
    { id: 'console', text: searchConsoleConnected ? 'Đã có dữ liệu Search Console import để dùng query/page.' : 'Chưa import dữ liệu Search Console, nên kiểm tra index và query thủ công.', level: searchConsoleConnected ? 'low' : 'medium' },
...
export function AiSeoScorePanelV4({ score }: { score: AiSeoScore }) {
  ...
  return <ModuleCard title="Điểm SEO AI v4" description="Điểm tự chấm theo dữ liệu thật. Dữ liệu Google chưa import là trạng thái chờ, không xem là lỗi nặng."><div className={styles.scoreHero}><strong>{score.overall}/100</strong><span>Điểm tổng thể</span></div><div className={styles.scoreList}>{rows.map(([label, value], index) => <div className={styles.scoreItem} key={`${label}-${index}`}><span>{label}</span><b>{value}</b><i><em style={{ width: `${value}%` }} /></i></div>)}</div><div className={styles.scoreStatus}><Badge status={score.details.sitemapOk ? 'ok' : 'warning'}>Sitemap {score.details.sitemapOk ? 'OK' : 'cần kiểm tra'}</Badge><Badge status={score.details.robotsOk ? 'ok' : 'warning'}>Robots {score.details.robotsOk ? 'OK' : 'cần kiểm tra'}</Badge><Badge status={score.details.searchConsoleConnected ? 'connected' : 'pending'}>Search Console {score.details.searchConsoleConnected ? 'đã kết nối' : 'pending'}</Badge><Badge status={score.details.googleAdsConnected ? 'connected' : 'pending'}>Google Ads {score.details.googleAdsConnected ? 'đã kết nối' : 'pending'}</Badge></div></ModuleCard>;
}
...
export function AiSeoScorePanelV41({ score }: { score: AiSeoScore }) {
  const rows = [['Technical SEO', score.technical, 'Sitemap, robots, canonical, 404'], ['Content', score.content, 'Sản phẩm, bài viết, cụm SEO, độ đầy dữ liệu'], ['Data', score.data, 'Supabase, keyword, cluster, URL'], ['Integration', score.integration, 'Search Console import/API, sitemap, robots']] as const;
  return <ModuleCard title="AI SEO Score v4.1" description="Điểm chi tiết hơn, Search Console có thể dùng dữ liệu import hoặc Query+Page API."><div className={styles.scoreHero}><strong>{score.overall}/100</strong><span>Điểm tổng thể</span></div><div className={styles.scoreList}>{rows.map(([label, value, detail], index) => <div className={styles.scoreItem} key={`${label}-${index}`}><span>{label}</span><b>{value}</b><small>{detail}</small><i><em style={{ width: `${value}%` }} /></i></div>)}</div><div className={styles.scoreStatus}><Badge status={score.details.supabaseOk ? 'ok' : 'error'}>Supabase</Badge><Badge status={score.details.sitemapOk ? 'ok' : 'warning'}>Sitemap</Badge><Badge status={score.details.robotsOk ? 'ok' : 'warning'}>Robots</Badge><Badge status={score.details.canonicalOk ? 'ok' : 'pending'}>Canonical</Badge><Badge status={score.details.searchConsoleConnected ? 'connected' : 'pending'}>Search Console import/API</Badge></div></ModuleCard>;
}
