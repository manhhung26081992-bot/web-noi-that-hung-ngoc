...
type V6Input = {
  ...
  searchConsole: SearchConsoleV5Data | null;
...
function uniqueBy<T>(items: T[], getKey: (item: T, index: number) => string) { const map = new Map<string, T>(); items.forEach((item, index) => { const itemKey = getKey(item, index); if (!map.has(itemKey)) map.set(itemKey, item); }); return Array.from(map.values()); }
...
function includesTerm(value: unknown, terms: string[]) {
  const clean = stripAccent(value);
  return terms.some((term) => clean.includes(stripAccent(term)));
}

function businessBoost(value: unknown) {
  const text = String(value || '');
  if (includesTerm(text, ['giường sắt', 'giường tầng sắt', 'giường tầng', 'giường'])) return 28;
  if (includesTerm(text, ['bàn làm việc', 'bàn văn phòng', 'bàn nhân viên', 'bàn chân sắt'])) return 28;
  if (includesTerm(text, ['bàn học sinh', 'bàn ghế học sinh', 'trường học', 'bảng từ'])) return 28;
  if (includesTerm(text, ['ghế chân quỳ', 'ghế giám đốc', 'tủ locker', 'tủ quần áo'])) return 6;
  return 0;
}

function detectedBusinessCluster(value: unknown) {
  const text = String(value || '');
  if (includesTerm(text, ['giường sắt', 'giường tầng sắt', 'giường tầng', 'giường'])) return 'Giường sắt';
  if (includesTerm(text, ['bàn làm việc', 'bàn văn phòng', 'bàn nhân viên', 'bàn chân sắt'])) return 'Bàn làm việc';
  if (includesTerm(text, ['bàn học sinh', 'bàn ghế học sinh', 'trường học', 'bảng từ'])) return 'Trường học';
  if (includesTerm(text, ['ghế chân quỳ'])) return 'Ghế chân quỳ';
  if (includesTerm(text, ['ghế giám đốc'])) return 'Ghế giám đốc';
  if (includesTerm(text, ['tủ locker'])) return 'Tủ locker';
  if (includesTerm(text, ['tủ quần áo'])) return 'Tủ quần áo';
  return '';
}

function isGenericCluster(value: unknown) {
  return includesTerm(value, ['cụm seo mới', 'cum seo moi', 'cụm seo chung', 'cum seo chung']) || !String(value || '').trim();
}

function safeClusterName(cluster: Pick<SeoCluster, 'name' | 'main_url'>, fallbackText = '') {
  const direct = String(cluster.name || '').trim();
  if (direct && !isGenericCluster(direct)) return direct;
  const detected = detectedBusinessCluster([cluster.main_url, fallbackText].join(' '));
  return detected || 'Nhóm sản phẩm chính';
}

function getImportedKeywordCount(data?: GoogleAdsImportData | null) {
  const rows = data?.rows || [];
  const unique = new Set(rows.map((row) => n(row.keyword)).filter(Boolean));
  if (unique.size) return unique.size;
  return Number(data?.summary?.keywordCount || 0);
}

function naturalDecisionTitle(cluster: string) {
  if (!cluster || isGenericCluster(cluster) || cluster === 'Nhóm sản phẩm chính') return 'Bổ sung dữ liệu cho nhóm sản phẩm chính';
  return 'Đẩy cụm ' + cluster;
}

function naturalSuggestion(title: string) {
  return title
    .replace(/Đẩy cụm\s+Cụm SEO mới/gi, 'Bổ sung dữ liệu cho nhóm sản phẩm chính')
    .replace(/Đẩy cụm\s+Cụm SEO chung/gi, 'Bổ sung dữ liệu cho nhóm sản phẩm chính')
    .replace(/Cụm SEO mới/gi, 'nhóm sản phẩm chính')
    .replace(/Cụm SEO chung/gi, 'nhóm sản phẩm chính');
}
...
function productsRank(products: ProductSeoItem[]) {
  return uniqueBy(products, (item, index) => key('product', item.id, item.slug, index))
    .map((product) => ({
      ...product,
      qualityScore: clamp(product.qualityScore ?? 0),
      issues: product.issues?.length ? product.issues : ['Chưa thấy vấn đề lớn'],
      action: product.action || 'Giữ ổn định, không đổi slug nếu đã index.',
    }))
...
function opportunities(input: V6Input, products: ProductSeoItem[], blogs: SeoBlogQualityItem[]): V6Opportunity[] {
  return input.clusters.map((cluster, index) => {
    const clusterName = safeClusterName(cluster);
    ...
    if (cluster.internal_link_measured === false) reasons.push('liên kết nội bộ chưa đo');
    if (!reasons.length) reasons.push(score >= 80 ? 'cụm đang ổn, ưu tiên theo dõi' : 'điểm chất lượng còn thấp');
    const nextAction = reasons[0].includes('bài viết')
      ? 'Viết/cập nhật bài hỗ trợ cho cụm ' + clusterName + '.'
      : reasons[0].includes('keyword')
        ? 'Thêm keyword chính và phụ cho cụm ' + clusterName + '.'
        : reasons[0].includes('sản phẩm')
          ? 'Bổ sung sản phẩm thật cho cụm ' + clusterName + ' nếu có hàng.'
          : 'Tăng liên kết nội bộ và FAQ cho cụm ' + clusterName + '.';
    return { id: String(cluster.id || cluster.name || index), cluster: clusterName, score, reasons, nextAction };
  }).sort((a, b) => (a.score - businessBoost(a.cluster) * 0.35) - (b.score - businessBoost(b.cluster) * 0.35));
}
...
function decisions(input: V6Input, products: ProductSeoItem[], blogs: SeoBlogQualityItem[], opps: V6Opportunity[]): V6Decision[] {
  ...
  if (c && c.score < 75) list.push({ id: 'p2-cluster-' + c.id, priority: 2, title: naturalDecisionTitle(c.cluster), reason: c.reasons.join(', '), action: c.nextAction, level: 'high', source: 'seo_clusters' });
...
function notifications(input: V6Input, products: ProductSeoItem[], blogs: SeoBlogQualityItem[], opps: V6Opportunity[]): V6Notification[] {
  ...
  const missingImage = products.filter((p) => p.issues.some((issue) => includesTerm(issue, ['ảnh', 'image', 'alt']))).length;
  ...
  if (weakBlog) list.push({ id: 'weak-blog', title: weakBlog + ' bài viết cần tối ưu', detail: 'Kiểm tra nội dung, ảnh, meta và liên kết nội bộ.', level: 'high', count: weakBlog });
...
function radar(input: V6Input, products: ProductSeoItem[], blogs: SeoBlogQualityItem[], opps: V6Opportunity[]): V6RadarPoint[] {
  ...
  const keyword = input.googleAdsV8?.summary.keywordCount ? clamp(Math.min((input.keywords.length + getImportedKeywordCount(input.googleAdsV8)) * 6, 100)) : (input.keywords.length ? clamp(Math.min(input.keywords.length * 8, 100)) : 35);
  ...
  return [{ label: 'Kỹ thuật', score: technical }, { label: 'Nội dung', score: content }, { label: 'Liên kết nội bộ', score: internalLink }, { label: 'Từ khóa', score: keyword }, { label: 'Sản phẩm', score: product }, { label: 'Cụm SEO', score: cluster }, { label: 'Bài viết', score: blog }, { label: 'Search Console', score: gsc }];
}
...
export function buildV6Analysis(input: V6Input): V6Analysis {
  ...
  const progress = {
    yesterday: input.logs.filter((log) => Date.parse(log.created_at || log.log_date) > Date.now() - 86400000).length,
    today: input.tasks.filter((task) => !task.completed && !['done', 'completed', 'hoàn thành'].includes(n(task.status))).length,
    sevenDays: input.logs.filter((log) => Date.parse(log.created_at || log.log_date) > Date.now() - 7 * 86400000).length,
    thirtyDays: input.logs.filter((log) => Date.parse(log.created_at || log.log_date) > Date.now() - 30 * 86400000).length,
    improved: [
      productRanking.filter((item) => (item.qualityScore || 0) >= 80).length + ' sản phẩm đạt điểm tốt',
      blogRanking.filter((item) => item.score >= 80).length + ' bài viết đạt điểm tốt',
      completedTasks ? completedTasks + ' việc đã hoàn thành' : 'Chưa có việc hoàn thành',
    ],
  };
...
export function AiRecommendationHistory({ decisions }: { decisions: V6Decision[] }) {
  ...
  function save(item: V6Decision) {
    const next = uniqueBy([{ id: item.id + '-' + Date.now(), date: new Date().toISOString(), suggestion: naturalSuggestion(item.title), done: false, result: naturalSuggestion(item.action) }, ...history], (row) => row.id).slice(0, 20);
    setHistory(next);
...
export function AiProgressEngine({ analysis }: { analysis: V6Analysis }) {
  ...
  return <ModuleCard title="Theo dõi tiến độ SEO" description="Theo dõi cải thiện hôm qua, hôm nay, 7 ngày và 30 ngày.">
    ...
    </div> : <EmptyState title="Chưa có dữ liệu tiến độ đủ để so sánh." detail="Khi có seo_logs, seo_progress hoặc việc hoàn thành, phần này sẽ tự tổng hợp." />}
...
export function SeoHealthRadar({ points }: { points: V6RadarPoint[] }) {
  ...
  return <ModuleCard title="Radar sức khỏe SEO" description="Radar tổng hợp Kỹ thuật, Nội dung, Liên kết nội bộ, Từ khóa, Sản phẩm, Cụm SEO, Bài viết và Search Console."><div className={styles.v6RadarWrap}><svg viewBox="0 0 240 240" className={styles.v6Radar} aria-label="Radar sức khỏe SEO"><polygon points={points.map((_, index) => { const angle = -Math.PI / 2 + index * (Math.PI * 2 / points.length); return String(120 + Math.cos(angle) * 105) + ',' + String(120 + Math.sin(angle) * 105); }).join(' ')} className={styles.v6RadarGrid} /><polygon points={polygon} className={styles.v6RadarShape} /></svg><div className={styles.v6RadarLegend}>{points.map((point, index) => <div key={key('radar', point.label, point.score, index)}><span>{point.label}</span><strong>{point.score}</strong></div>)}</div></div></ModuleCard>;
}

export function AutoInsightPanel({ insights }: { insights: string[] }) {
  return <ModuleCard title="Nhận xét tự động" description="Mỗi lần làm mới, AI sinh 3 nhận xét từ dữ liệu hiện tại.">{insights.length ? <div className={styles.v6List}>{insights.map((item, index) => <div className={styles.v6Insight} key={key('insight', item, '', index)}>{item}</div>)}</div> : <EmptyState title="Chưa đủ dữ liệu insight" detail="Thêm sản phẩm, blog hoặc cluster để AI tự nhận xét." />}</ModuleCard>;
}
...
function TodaySeoFocusV61Base({
  ...
  const todayItems = useMemo(() => uniqueText([
    ...decisions.map((item) => naturalSuggestion(item.title) + ': ' + naturalSuggestion(item.action)),
    ...commands.map((item) => item.title + ': ' + item.detail),
    ...dailyBrief.map((item) => item.text),
    ...pendingTasks.map((item) => 'Làm việc: ' + item.title),
  ]).slice(0, 6), [commands, dailyBrief, decisions, pendingTasks]);

  return (
    <ModuleCard title="Hôm nay cần làm gì" description={'Gộp AI Daily Brief, Daily SEO Mission, Today Task, Nhận xét tự động và Smart Notification. Cập nhật: ' + lastUpdated}>
