...
export function buildSeoCommands({ overview, tasks, health, keywords, searchConsoleConnected }: { overview: SeoOverview | null; tasks: TodayTask[]; health: SeoHealthSnapshot | null; keywords: SeoKeyword[]; searchConsoleConnected: boolean }): SeoCommand[] {
  ...
  commands.push(searchConsoleConnected ? { id: 'console-ok', title: 'Đọc dữ liệu Search Console import', detail: 'Dashboard đang dùng dữ liệu Search Console import thủ công để theo dõi impression, click và query.', level: 'medium', source: 'search_console' } : { id: 'console', title: 'Import Search Console thủ công', detail: 'Chưa import dữ liệu Search Console; có thể kiểm tra index và query thủ công rồi dán dữ liệu vào dashboard.', level: 'medium', source: 'search_console' });
...
export function buildAiInsights({ overview, health, tasks, logs, searchConsoleConnected }: { overview: SeoOverview | null; health: SeoHealthSnapshot | null; tasks: TodayTask[]; logs: SeoLog[]; searchConsoleConnected: boolean }): AiInsight[] {
  ...
  return [
    ...
    { id: 'console', text: searchConsoleConnected ? 'Đã có dữ liệu Search Console import, có thể dùng impression/click để ra quyết định.' : 'Chưa nhập dữ liệu Search Console nên chưa có impression/click thật; hiện vẫn nên kiểm tra thủ công.', level: searchConsoleConnected ? 'low' : 'medium' },
...
export function AiSeoScorePanel({ score }: { score: AiSeoScore }) {
  ...
  return <ModuleCard title="Điểm SEO AI" description="Điểm tự chấm thực tế từ tình trạng website. Dữ liệu Google chưa import được xem là trạng thái chờ."><div className={styles.scoreHero}><strong>{score.overall}/100</strong><span>Điểm tổng thể</span></div><div className={styles.scoreList}>{rows.map(([label, value]) => <div className={styles.scoreItem} key={label}><span>{label}</span><b>{value}</b><i><em style={{ width: `${value}%` }} /></i></div>)}</div><div className={styles.scoreStatus}><Badge status={score.details.searchConsoleConnected ? 'connected' : 'pending'}>Search Console {score.details.searchConsoleConnected ? 'đã kết nối' : 'pending'}</Badge><Badge status={score.details.googleAdsConnected ? 'connected' : 'pending'}>Google Ads {score.details.googleAdsConnected ? 'đã kết nối' : 'pending'}</Badge><Badge status={score.details.canonicalOk ? 'ok' : 'pending'}>Canonical {score.details.canonicalOk ? 'OK' : 'pending'}</Badge></div></ModuleCard>;
}
